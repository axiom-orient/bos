...
- `sku`
  ...
  - `companyName`이 없으면 bundle id prefix seed를 사용해 자동 생성
...
- 대상 TASK-ID: `QA-011`, `DOC-010`, `REL-009`
- 산출물:
  - integration tests
...
- 목적: 제품 release verdict에 필요한 live read-only evidence와 운영용 first writable seed를 분리한다.
- 실제로 완료한 시나리오:
  - temp workspace 준비
...
- 운영용 후속 시나리오:
  ...
  - 이 단계는 제품 release 게이트가 아니라 첫 팀/app signing seed bootstrap 절차로 남긴다.
...
- first signing seed(`release-check --mode sync-certs --allow-write`)는 운영용 bootstrap으로 남기고, 제품 release verdict에서는 제외한다.

## Risk/Rollback
- Risk: App Store Connect API role/권한 차이 때문에 live create path가 계정마다 다를 수 있다.
  - Mitigation: safe existing-path 검증과 제품 release verdict를 분리하고, writable seed는 운영 runbook으로 남긴다.
